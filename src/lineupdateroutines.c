...
MAP_ERROR_CODE set_line_variables_pre_solve(ModelData* data, char* map_msg, MAP_ERROR_CODE* ierr)
{
  ...
  while (list_iterator_hasnext(&data->element)) { /* tell whether more values available */ 
    ...
    iterElement->l.value = set_horizontal_excursion(iterElement);
    
    /* vertical cable excursion */
    iterElement->h.value = set_vertical_excursion(iterElement);

    /* angle of azimuth */
    success = set_psi(iterElement, map_msg, ierr); 
    if (success!=MAP_SAFE) {
      cx = map_snprintf(buffer, 64, "Element number: %d.", i); assert(cx>=0);
      *ierr = map_set_universal_error(buffer, map_msg, ierr, MAP_WARNING_6);
    };
    i++;
  };
  list_iterator_stop(&data->element); /* ending the iteration "session" */    
